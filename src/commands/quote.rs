use serenity::{all::ChannelId, builder::{CreateEmbed, CreateEmbedAuthor, CreateMessage}};
use super::{Context, Error};
use serenity::all::Message;

async fn quote_internal(ctx: Context<'_>, author: &String, quote: &String, icon_url: Option<String>) -> Result<(), Error> {
...
pub async fn quote(
    ...
    quote: String,
    #[description = "The author of said text"]
    author: String,
) -> Result<(), Error> {
...
pub async fn context_quote(
    ctx: Context<'_>,
    msg: Message,
) -> Result<(), Error> {
    quote_internal(ctx, &msg.author.name, &msg.content, 
        Option::Some(msg.author.avatar_url().unwrap_or(msg.author.default_avatar_url()))).await
}